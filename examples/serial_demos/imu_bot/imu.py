import math
from atlasbuggy.microcontrollers import SerialObject
...
class IMU(SerialObject):
    def __init__(self, enabled=True):
        ...
        super(IMU, self).__init__("BNO055-IMU", enabled)
            ...
            for segment in data:
                if len(segment) > 0:
                    if segment[0] == "e":
                        ...
                        self.logger.error("IMU: Invalid segment type: '%s'. Data: '%s'" % (segment[0], data))
                else:
                    self.logger.error("IMU: Empty segment! '%s'" % data)
        except ValueError:
